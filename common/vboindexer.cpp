...
)
{
    ...
    // It looks like we'll have to add it to the VBO.
    return false;
}
...
)
{
    ...
    for (unsigned int i = 0; i < in_vertices.size(); i++)
    {
        ...
        else
        {
            ...
            out_uvs.push_back(in_uvs[i]);
            out_normals.push_back(in_normals[i]);
            unsigned short outVertexIndex = static_cast<unsigned short>(out_vertices.size()) - 1;
            out_indices.push_back(outVertexIndex);
            VertexToOutIndex[packed] = outVertexIndex;
        }
    }
}
