...
void printText2D(const char* text, const int x, const int y, const int size)
{
    ...
    for (unsigned int i = 0; i < length; i++)
    {
        ...
        auto uv_up_left = glm::vec2(uv_x, uv_y);
        auto uv_up_right = glm::vec2(uv_x + 1.0f / 16.0f, uv_y);
        auto uv_down_right = glm::vec2(uv_x + 1.0f / 16.0f, (uv_y + 1.0f / 16.0f));
        auto uv_down_left = glm::vec2(uv_x, (uv_y + 1.0f / 16.0f));
