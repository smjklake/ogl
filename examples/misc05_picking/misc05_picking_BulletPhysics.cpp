...
int main()
{
	...
	window = glfwCreateWindow( 1024, 768, "Misc 05 - Bullet version", nullptr, nullptr);
	if( window == nullptr){
	...
	for(int i=0; i<100; i++){
		...
		orientations[i] = normalize(quat(vec3(rand()%360, rand()%360, rand()%360)));
   ...
   // Build the broadphase
	...
	for(int i=0; i<100; i++){
		...
		btRigidBody::btRigidBodyConstructionInfo rigidBodyCI(
			0,                  // mass, in kg. 0 -> a Static object will never move.
			motionstate,
