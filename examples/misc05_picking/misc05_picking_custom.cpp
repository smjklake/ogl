...
void ScreenPosToWorldRay(
	int mouseX, int mouseY,             // Mouse position, in pixels, from the bottom-left corner of the window
	...
	vec3& out_origin,              // Output: Origin of the ray. /!\ Starts at the nearest plane, so if you want the ray to start at the camera's position instead, ignore this.
	vec3& out_direction            // Output: Direction, in world space, of the ray that goes "through" the mouse.
){
...
bool TestRayOBBIntersection(
	...
	// Test intersection with the 2 planes perpendicular to the OBB's X axis
	{
		...
		if (float f = dot(ray_direction, xaxis); fabs(f) > 0.001f ){ // Standard case
	...
	// Exactly the same thing as above.
	{
		...
		if (float f = dot(ray_direction, yaxis); fabs(f) > 0.001f ){
	...
	// Exactly the same thing as above.
	{
		...
		if (float f = dot(ray_direction, zaxis); fabs(f) > 0.001f ){
...
int main( void )
{
	...
	do{
		...
		if (glfwGetMouseButton(window, GLFW_MOUSE_BUTTON_LEFT)){
			...
			// because it already has some spatial partitioning structure,
			// like Binary Space Partitioning Tree (BSP-Tree),
