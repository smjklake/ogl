...
int main( void )
{
	...
	window = glfwCreateWindow( 1024, 768, "Tutorial 09 - Loading with AssImp", nullptr, nullptr);
	if( window == nullptr){
	...
	const GLuint programID = LoadShaders( "StandardShading.vert", "StandardShading.frag" );
	...
	const std::vector<vec3> indexed_vertices;
	const std::vector<vec2> indexed_uvs;
	const std::vector<vec3> indexed_normals;
